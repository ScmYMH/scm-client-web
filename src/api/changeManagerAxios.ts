...
export async function putCntrtChgInfo(cntrtIdList: string[]) {
	const response = await axios.put<boolean>(`http://localhost:9091/contract/manager/chginfo`, cntrtIdList);
	return response.data;
}
