...
function* postContractMemberSaga(action: ReturnType<typeof postUserMemberAsync.request>) {
	try {
		const postcontractmemberInfo: Array<ContractMemberInfo> = yield call(postContractMemberList, action.payload);
...
function* deleteContractMemberSaga(action: ReturnType<typeof deleteContractMemberAsync.request>) {
	try {
		const deletecontractmemberInfo: String = yield call(deleteContractMemberList, action.payload);
